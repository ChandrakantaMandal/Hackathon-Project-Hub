...
const ProjectPage = () => {
  ...
  const getFilteredTasks = () => {
    ...
    if (isProjectOwner()) {
      ...
      return project.tasks.filter((task) => task.assignedTo?._id === user?._id);
    }
  };
  ...
  const handleTaskStatusToggle = async (taskId, currentStatus) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.success(
          `Task marked as ${newStatus === "completed" ? "completed ✅" : "incomplete 📝"}`,
        );
      }
    } catch (error) {
      ...
      <div className="glass-card p-6">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div className="text-sm text-gray-600">
            {project.showcase?.isPublic ? "Public on Showcase" : "Private"} •{" "}
            {project.showcase?.views || 0} views
          </div>
        </div>
      </div>
                          ...
                          {!isProjectOwner() &&
                            task.assignedTo?._id === user?._id && (
                              <input
                                ...
                                onChange={() =>
                                  handleTaskStatusToggle(task._id, task.status)
                                }
                                className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 dark:bg-slate-700 dark:border-slate-500"
                              />
                            )}
                          <h4
                            className={`font-medium ${task.status === "completed" ? "line-through text-gray-500 dark:text-slate-400" : "text-gray-900 dark:text-slate-100"}`}
                          >
                            {task.title}
                          </h4>
                        </div>
              ...
              {getAllMembersList().length > 0 ? (
                getAllMembersList().map((member, index) => (
                  <img
                    ...
                    src={
                      member.avatar ||
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(member.name || "User")}&background=8B5CF6&color=ffffff&size=128&bold=true`
                    }
        ...
        <div className="space-y-6">
          <div className="glass-card p-6">
            ...
            <p className="text-gray-600">
              Likes: {project.showcase?.likes?.length || 0}
            </p>
            <p className="text-gray-600">
