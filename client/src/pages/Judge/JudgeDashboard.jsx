...
const JudgeDashboard = () => {
  ...
  const badges = [
    {
      ...
      description: "First team to submit",
    },
    {
      type: "last-arrival",
      name: "The Last Arrival",
      description: "Last team to submit",
    },
    {
      type: "innovation-master",
    ...
    },
    {
      type: "design-guru",
      name: "Design Guru",
      description: "Outstanding UI/UX design",
    },
    {
      type: "peoples-choice",
  ...
  const handleScore = async () => {
    try {
      ...
      await api.post(
        `/judge/submissions/${selectedSubmission._id}/score`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleAwardBadge = async (badgeType) => {
    try {
      ...
      await api.post(
        `/judge/submissions/${selectedSubmission._id}/badge`,
        badge,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleRemoveBadge = async (submissionId, badgeIndex) => {
    try {
      const token = localStorage.getItem("judgeToken");
      await api.delete(
        `/judge/submissions/${submissionId}/badge/${badgeIndex}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
                  ...
                  <p className="text-2xl font-bold text-white">
                    {
                      submissions.filter((s) =>
                        s.scores?.some(
                          (score) => score.judge?._id === judge._id,
                        ),
                      ).length
                    }
                  </p>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-2xl font-bold text-white">
                    {submissions.reduce(
                      (total, s) => total + (s.badges?.length || 0),
                      0,
                    )}
                  </p>
          ...
          {!loading && submissions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">
                Submissions will appear here once teams start submitting their
                projects
              </p>
            </div>
                    ...
                    <div className="flex gap-2">
                      {submission.liveLink && (
                        <Button
                          size="small"
                          onClick={() =>
                            window.open(submission.liveLink, "_blank")
                          }
                      ...
                      <div className="flex flex-wrap gap-2">
                        {submission.badges.map((badge, index) => (
                          ...
                          >
                            ...
                            <button
                              onClick={() =>
                                handleRemoveBadge(submission._id, index)
                              }
                    ...
                    {judgeScore && (
                      <div className="text-right">
                        <p className="text-white font-semibold">
                          Your Score:{" "}
                          {(
                            (judgeScore.innovation +
                              judgeScore.technical +
                              judgeScore.design +
                              judgeScore.presentation +
                              judgeScore.overall) /
                            5
